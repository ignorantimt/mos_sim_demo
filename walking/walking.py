...
class walking():
  def __init__(self, RobotId, left_foot0, right_foot0, joint_angles, pc):
    self.kine = THMOSLegIK()
    self.left_foot0, self.right_foot0 = left_foot0, right_foot0
    self.joint_angles = joint_angles
    self.pc = pc
    self.fsp = foot_step_planner(0.05, 0.03, 0.2, 0.3, foot_width)  # period 0.34
    ...
    if pos == None:
      ...
      if len(self.foot_step) > 2:
        if not self.status == 'start':
          offset_y = -foot_width if self.next_leg == 'left' else foot_width
        else:
          offset_y = 0.0
...
#    print(str(self.foot_step)+'\n')
    ...
    if self.foot_step[0][4] == 'left':
      if  self.foot_step[1][4] == 'both':
        ...
        self.right_off_g = np.matrix([[self.foot_step[1][1], self.foot_step[1][2]+foot_width, self.foot_step[1][3]]])
    ...
    if self.foot_step[0][4] == 'right':
      if  self.foot_step[1][4] == 'both':
        ...
        self.left_off_g  = np.matrix([[self.foot_step[1][1], self.foot_step[1][2]-foot_width, self.foot_step[1][3]]])
